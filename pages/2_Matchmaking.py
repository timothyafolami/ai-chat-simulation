...
st.caption("Choose a persona on the sidebar, then view side‑by‑side previews with top matches.")

# Sidebar controls
sb = st.sidebar
sb.title("Matchmaking – Controls")
source_choice = sb.radio("Query source", ["Local persona JSON", "Upload JSON"], index=0)
...
with results_area:
    if go:
        ...
        if not matches:
            ...
            base_obj = query_obj if source_choice == "Upload JSON" else load_persona_json(GENERATED_DIR / selected_file)
            p1_avatar = "🟦"
            p2_avatar = "🟩"
            ...
            for rank, item in enumerate(matches, start=1):
                ...
                with st.container(border=True):
                    ...
                    st.caption(f"combined_score={score:.4f}")
